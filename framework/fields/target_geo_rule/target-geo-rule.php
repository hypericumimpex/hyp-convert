...
function cp_v2_target_geo_rule_settings_field( $name, $settings, $value ) {
	...
	if ( isset( $settings['conditional_logic'] ) && $settings['conditional_logic'] ) {
		...
		$output       .= '<div class="checkbox cp-allow-specific-countries-posts"><p><label><input type="checkbox" value="" id="" class="" name ="" ' . $checked . '> ' . $settings['conditional_label'] . '</label></p></div>';
	}
		...
		if ( null != $data['specific'] && is_array( $data['specific'] ) ) {
			...
			foreach ( $data['specific'] as $data_key => $sel_value ) {
				...
				foreach ( $arr_all_countries as $key => $value ) {
					if ( $sel_value === $key ) {
						$output .= '<option value="' . $sel_value . '" selected="selected" >' . $value . '</option>';
					}
				}
			}
		}
