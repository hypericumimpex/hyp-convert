...
$action = ( isset( $_GET['action'] ) && 'install' === $_GET['action'] ) ? $_GET['action'] : '';
if ( 'install' === $action ) {
	...
	if ( '' !== $request_product_id ) {
		?>
			<div class="clear"></div>
			<div class="wrap">
			<h2><?php _e( 'Installing Addon', 'convertpro' ); ?></h2>
			<?php
			...
			<?php if ( isset( $installed['status'] ) && true === $installed['status'] ) : ?>
					...
					<a href="<?php echo ( is_multisite() ) ? network_admin_url( 'plugins.php#' . $current_name ) : admin_url( 'plugins.php#' . $current_name ); ?>"><?php _e( 'Manage plugin here', 'convertpro' ); ?></a>
				<?php endif; ?>
				</div>
			<?php
...
?>
<div class="clear"></div>
<div class="bsf-sp-screen <?php echo 'extension-installer-' . $product_id; ?>">

	<div class="cp-addon-wrap">
		<h3 class="bf-ext-sub-title cp-gen-set-title"><?php echo __( 'Available Addons', 'convertpro' ); ?></h3>
						...
						<span class="cp-ext-inner">
							...
							<div class="top-section">
								...
								if ( ! $is_plugin_installed ) {
									...
									$current_name = preg_replace( '![^a-z0-9]+!i', '+', $current_name );
									if ( is_multisite() ) {
										$link = network_admin_url( 'plugins.php?s=' . $current_name );
									} else {

										$link = admin_url( 'plugins.php?s=' . $current_name );
								...
								?>
								<a target="_blank" rel="noopener" class="cp-btn-primary cp-md-btn cp-button-style cp-addon-btn extension-button <?php echo $button_class; ?>" href="<?php echo $link; ?>" data-ext="<?php echo $key; ?>"><?php echo $button; ?></a>
							</div>
								</div>
							</div>
						</span>
	...
	<?php else : ?>
		<div class="bsf-extensions-no-active">
			...
			<p class="bsf-text-light"><em><?php echo __( 'No addons available yet!', 'convertpro' ); ?></em></p>

			<div class="bsf-cp-rem-bundle" style="margin-top: 30px;">
				<a class="button-primary" href="<?php echo esc_url( $reset_bundled_url ); ?>">
					<?php _e( 'Refresh Bundled Addons', 'convertpro' ); ?>
				</a>
			</div>
		</div>
